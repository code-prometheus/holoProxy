...
impl StreamContext {
    ...
    fn finalize_intercept(&mut self) {
        while self.intercept_active {
            ...
            // ★ BUG-FIX #1: 区分"找到闭合标签"与"流被截断"
            let (full_xml, remaining, found_close) = if let Some(orig_end) =
                find_close_tag_end_in_original(&buffer, &norm_buf, &self.active_close_tag)
            {
                (buffer[..orig_end].to_string(), buffer[orig_end..].to_string(), true)
            } else {
                (buffer, String::new(), false)
            };
            ...
            if found_close {
                ...
                // ★ 没有闭合标签 = 流被截断，绝不构造 tool_use，
                //   保持 has_tool_use=false 让 finish() 里的 recovery 接管
                warn!(
                    "⚠️ [XML Parse] finalize_intercept 未找到闭合标签（流被截断），丢弃不完整工具块，交给 recovery"
                );
            }
    ...
    fn check_text_buffer_triggers(&mut self) {
        ...
        if let Some((idx, tag_base)) = trigger_info {
            ...
            if len > 50 {
                ...
                let cut_pos = self.text_buffer[safe_cut..]
                    .rfind('<')
                    .map(|i| safe_cut + i)
                    .unwrap_or(safe_cut);
    ...
    pub fn handle_tool_call(&mut self, tc: &OpenAIToolCallDelta) {
        ...
        if !self.active_native_tools.contains_key(&idx) {
            ...
            // ★ BUG-FIX #3: 无名称的 tool_call delta 是截断/异常产物，
            //   不能 open_tool，否则 has_tool_use=true 会堵死 recovery
            if name.is_empty() {
                warn!(
                    "[{}] ⚠️ native tool_call delta without name (truncated?), skip open_tool",
                    self.msg_id
                );
                return;
            }
    ...
    pub async fn finish(&mut self, upstream_stop_reason: &str) {
        ...
        if self.is_agent_mode && !self.has_tool_use {
            // ★ BUG-FIX #2: timeout 5s→15s；超时改为保守恢复，不再静默放弃
            ...
            let need_recovery: Option<String> = match recovery_result {
                Ok(Some(reason)) => Some(reason),
                Ok(None) => None,
                Err(_) => {
                    warn!(
                        "[{}] ⏰ Recovery check timed out after 15s, conservative recovery",
                        self.msg_id
                    );
                    Some("recovery check timed out, conservative recovery".into())
                }
            };

            if let Some(reason) = need_recovery {
